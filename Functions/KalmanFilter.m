function [logL, table_ll, Q, table_at_filter, table_at_prediction, table_at_smoother, ft] = KalmanFilter(par, yt, T, delivery_time, dt, smoothing, seasonality)
...
% Initial a and P
at_prediction = [ 0 ; muxi / gamma ]; % a_1|0
...
for j = 1:nT
    ...
    table_ll(j) = -(0.5*length( yt(j, ~isnan(yt(j,:))) )*log(2*pi) + 0.5*log(det(L)) + 0.5*et'/L*et);
end
